...
function patchStagedDockerfile(
  ...
  const sandboxBaseImageEnv = (process.env.NEMOCLAW_BASE_IMAGE || "").trim();
  if (sandboxBaseImageEnv) {
    dockerfile = dockerfile.replace(/^ARG BASE_IMAGE=.*$/m, `ARG BASE_IMAGE=${sandboxBaseImageEnv}`);
  }
